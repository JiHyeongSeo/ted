...
func (e *Editor) Run(screen tcell.Screen) error {
	...
	// If no tabs open, create empty buffer
	if e.tabs.Count() == 0 {
		e.OpenEmpty()
	}
...
func (e *Editor) handleKeyEvent(ev *tcell.EventKey) {
	...
	case input.ResolveMatched:
		e.commands.Execute(cmd, e)
		return
	case input.ResolvePending:
		return // waiting for chord completion
	}
...
func (e *Editor) LoadKeybindings() {
	...
	e.keymap.Bind("ctrl+tab", "tab.next", "")
	e.keymap.Bind("ctrl+shift+tab", "tab.previous", "")
